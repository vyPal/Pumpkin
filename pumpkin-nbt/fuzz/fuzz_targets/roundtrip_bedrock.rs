...
use pumpkin_nbt::deserializer::from_bytes_bedrock;
use pumpkin_nbt::serializer::to_bytes_bedrock;
...
fuzz_target!(|data: RoundtripStruct| {
    ...
    if to_bytes_bedrock(&fixed_data, &mut bytes).is_ok() {
        let cursor = Cursor::new(bytes);
        let reconstructed: RoundtripStruct =
            from_bytes_bedrock(cursor).expect("Failed to deserialize valid NBT");
